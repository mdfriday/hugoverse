...
type BackupScheduler struct {
	...
	systemDBPath  string
	contentServer *contentEntity.Content // 用于获取 license 数据
...
func (bs *BackupScheduler) backupCouchDB(tempDir string, info *CouchDBBackupInfo) error {
	...
	// 获取付费用户的数据库列表（只备份付费用户）
	paidUserDBs := bs.getPaidUserDatabases()
	bs.log.Printf("Found %d paid user database(s) to backup", len(paidUserDBs))
	...
	bs.log.Printf("Total databases: %d, Target databases for backup: %d (%d paid users + _users)",
		len(databases), len(targetDatabases), len(targetDatabases)-1)
	...
	bs.log.Printf("CouchDB backup completed: %d databases (paid users only), %d docs, %.2f MB",
